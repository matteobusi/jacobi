 ...
 */
/*
 * Simple work-flow computation on 1-D arrays.
 *
 * Input: A, B, C, D arrays of size SIZE
...
const bool check = true;    // true to check the result
long SIZE = (1 << 20);
...
int main(int argc, char *argv[]) {
    ...
    if (check) {
        ...
        printf("\nRunning sequential computation for checking the result:\n");
        // re-init data
        for(long i=0;i<SIZE;++i) {
            A[i] = 0; B[i] = i;
            C[i] = 1; D[i] = i+1;
