...
bool PFSPINWAIT=false;      // enable/disable spinWait
...
void blockMM(long b, const double *A, const long AN, 
    ...
    pf.parallel_for(0,b,1,PFGRAIN,[A,B,AN,BN,CN,b,&C](const long i) {
            for (long j = 0; j < b; j++) {
                for (long k = 0; k < b; k++)  
                    C[i*CN+j] += A[i*AN+k]*B[k*BN+j];  
            }  
...
void taskGen(Parameters<ff_mdf > *const P){
    ...
    double       *C = P->C;
    long m=P->m,n=P->n,p=P->p,b=P->b;
    long AN=P->AN,BN=P->BN,CN=P->CN;
	long mblocks = m/b, nblocks = n/b, pblocks = p/b;
    auto mdf = P->mdf;
    ...
    for(long i=0; i<mblocks; ++i) {
        for(long j=0; j<nblocks; ++j) {
            ...
            const param_info _1={(uintptr_t)Cij, OUTPUT};
            Param.push_back(_1);
            mdf->AddTask(Param, zerosBlock, b, Cij, CN);
        }
    }
    ...
    for(long k=0; k<pblocks; ++k) {
        for(long i=0; i<mblocks; ++i) {
            for(long j=0; j<nblocks; ++j) {
                ...
                double *Cij       = &C[b*(i*CN+j)];
                std::vector<param_info> Param;
                const param_info _1={(uintptr_t)Cij, INPUT};
                Param.push_back(_1); 
                mdf->AddTask(Param, blockMM, b, Aik, AN, Bkj, BN, Cij, CN);
            }
        }
    }
...
void get_time( void (*F)(long b, long m, long n, long p, const double* A, long AN, const double* B, long BN, double* C, long CN),
...
void get_time_and_check(void (*F)(long b, long m,long n,long p, const double* A, long AN, const double* B, long BN, double* C, long CN),
                        long b, long m,long n,long p, const double* A, long AN, const double* B, long BN, const double *C_expected, double *C,
                        const char *descr) {
    get_time(F, b, m, n, p, A, AN, B, BN, C, descr);
...
int main(int argc, char* argv[]) {     
    if (argc < 5) {
        printf("\n\tuse: %s <M> <N> <P> <blocksize> [nworkers] [pfworkers:pfgrain] [check=0]\n", argv[0]);
    ...
    ff_mdf mdf(taskGen, &Param, 128, (nw<=0 ? ff_realNumCores(): nw));
    Param.m=M,Param.n=N,Param.p=P,Param.b=b;
    Param.AN=P,Param.BN=N,Param.CN=N;
    Param.mdf=&mdf;
    
    if (check) {
        double *C2   = (double*)malloc(M*N*sizeof(double));
        get_time(seqMatMult, 1, M,N,P, A, P, B, N, C, "Standard ijk algorithm");
        Param.C=C2;        
        printf("Executing %-40s", "Block-based MM");
        ffTime(START_TIME);
