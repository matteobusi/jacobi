 ...
 * 
 *    farm( Read+Scheduler, BlurFilter+EmbossFilter+Write)
 *  
 *
 *                    ---> BlurFilter + EmbossFilter + Write
 *                   |                                  
...
struct BlurEmbossWriteWrapper: ff_node_t<Task> {
    Task *svc(Task *task) {
        return Write(EmbossFilter(BlurFilter(task,this),this),this);
    }
};
...
int main(int argc, char *argv[]) {
    ...
    for(int i=0;i<Wrks;++i) 
        W.push_back(std::unique_ptr<ff_node_t<Task> >(new BlurEmbossWriteWrapper));
    
    Read read(&argv[start], num_images, radius, sigma);
    ff_Farm<Task> farm(std::move(W), read);
