 ...
 * 
 *    pipeline( Read, BlurFilter, EmbossFilter, Write )
 *  
 *      Read ---> BlurFilter ---> EmbossFilter ---> Write
 *
 *  command: 
 *
 *    img_pipe -r 1.5 -s 0.6 ./in\/*
 *
...
int main(int argc, char *argv[]) {
    ...
    Read read(&argv[start], num_images, radius, sigma);  // 1st stage
    ff_node_F<Task> blur(BlurFilter);                    // 2nd stage
    ff_node_F<Task> emboss(EmbossFilter);                // 3nd stage
    ff_node_F<Task> write(Write);                        // 4th stage
    ff_Pipe<> pipe(read,blur,emboss,write);
