...
/*  parallel task-farm version of the 'img' program
 * 
 *    farm( ReadFileName+Scheduler, ReadBlurFilter+EmbossFilter+Write)
 *  
 *
 *                           ----> Read + BlurFilter + EmbossFilter + Write
 *                          |                                  
 *   ReadFileName+Scheduler-|----> Read + BlurFilter + EmbossFilter + Write
 *                          |                                  
 *                           ----> Read + BlurFilter + EmbossFilter + Write
 *  
 *  command: 
 *
 *    img_farm3 -r 1.5 -s 0.6 -n 2 ./in\/*
 *
...
// 1st stage
struct ReadFileName: ff_node_t<Task> {
    ReadFileName(char **images, const long num_images, double r, double s):
        images((const char**)images),num_images(num_images),radius(r),sigma(s) {}

    Task *svc(Task *) {
        for(long i=0; i<num_images; ++i) {
            Task *t = new Task(nullptr, images[i],radius,sigma);
...
Task* Read(Task *in, ff_node*const) {
    const std::string &filepath(in->name);
    std::string filename;
...
struct ReadBlurEmbossWriteWrapper: ff_node_t<Task> {
    Task *svc(Task *task) {
        return Write(EmbossFilter(BlurFilter(Read(task,this),this),this),this);
    }
};
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        std::cerr << "use: " << argv[0] << 
            " [-r radius=1.0] [-s sigma=.5] [ -n Wrks=2] <image-file> [image-file]\n";
        return -1;
    }
    ...
    for(int i=0;i<Wrks;++i) 
        W.push_back(std::unique_ptr<ff_node_t<Task> >(new ReadBlurEmbossWriteWrapper));
    
    ReadFileName readfn(&argv[start], num_images, radius, sigma);
    ff_Farm<Task> farm(std::move(W), readfn);
    farm.set_scheduling_ondemand();
    farm.remove_collector();

    if (farm.run_and_wait_end()<0) { // executes the task-farm
