 ...
 * 
 *    farm( Read+Scheduler, BlurFilter+EmbossFilter, Collector+Write)
 *  
 *
 *                    ---> BlurFilter + EmbossFilter --
 *                   |                                  |
 *    Read+Scheduler ----> BlurFilter + EmbossFilter ---->Collector+Write
 *                   |                                  |
 *                    ---> BlurFilter + EmbossFilter --
 *
 *  command: 
 *
 *    img_farm -r 1.5 -s 0.6 -n 2 ./in\/*
 *
...
struct BlurEmbossWrapper: ff_node_t<Task> {
    Task *svc(Task *task) {
        return EmbossFilter(BlurFilter(task,this),this);
    }
};
...
int main(int argc, char *argv[]) {
    ...
    for(int i=0;i<Wrks;++i) 
        W.push_back(make_unique<BlurEmbossWrapper>());

    Read read(&argv[start], num_images, radius, sigma);
    Writer writer;        
    ff_Farm<Task> farm(std::move(W), read, writer);
